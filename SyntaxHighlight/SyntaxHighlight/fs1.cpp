...
CPluginInterfaceForFS::ExecuteChangeDriveMenuItem(int panel)
{
  ...
  while (1)
  {
                       ...
                       SalamanderGeneral->GetMsgBoxParent(), ConnectDlgProc, NULL) == IDOK)
    {
      ...
      if (!changeRes && failReason == CHPPFR_INVALIDPATH) continue;  // zopakujeme si zadani
      */
/*
                                                      ...
                                                      BOOL &closeMenu, int &postCmd, void *&postCmdParam)
{
  ...
  // vytvorime menu
  char **strings;
  char *strings1[] = {"&Show in Panel", "&Disconnect", NULL}; // polozky pro odpojeny plugin FS
  char *strings2[] = {"&Refresh", "&Disconnect", NULL};       // polozky pro aktivni plugin FS
  char *strings3[] = {"Connect To...", NULL};                 // polozky pro FS
  if (pluginFS != NULL)
  {
    if (isDetachedFS) strings = strings1;
    else strings = strings2;
  }
  else strings = strings3;
  ...
  else return FALSE;  // cancel nebo chyba menu
}
  ...
  if (isDir)  // podadresar nebo up-dir
  {
    ...
    if (isDir == 2)   // up-dir
    {
      ...
      if (SalamanderGeneral->CutDirectory(newPath, &cutDir))  // zkratime cestu o posl. komponentu
      {
        ...
        if (!fs->TopIndexMem.FindAndPop(newPath, topIndex)) topIndex = -1;
...
void
CTopIndexMem::Push(const char *path, int topIndex)
{
  ...
  if (ok)   // navazuje -> zapamatujeme si dalsi top-index
  {
    ...
    {
      int i;
      for (i = 0; i < TOP_INDEX_MEM_SIZE - 1; i++) TopIndexes[i] = TopIndexes[i + 1];
      TopIndexesCount--;
    }
    strcpy(Path, path);
...
BOOL
CTopIndexMem::FindAndPop(const char *path, int &topIndex)
{
  // zjistime, jestli path odpovida Path (path==Path)
  int l1 = (int)strlen(path);
  if (l1 > 0 && path[l1 - 1] == '\\') l1--;
  int l2 = (int)strlen(Path);
  if (l2 > 0 && Path[l2 - 1] == '\\') l2--;
...
CPluginFSDataInterface::ColumnFixedWidthShouldChange(BOOL leftPanel, const CColumn *column, int newFixedWidth)
{
  if (leftPanel)
  {
    ...
    {
      case 1: CreatedFixedWidth = MAKELONG(newFixedWidth, HIWORD(CreatedFixedWidth)); break;
      case 2: ModifiedFixedWidth = MAKELONG(newFixedWidth, HIWORD(ModifiedFixedWidth)); break;
      case 3: AccessedFixedWidth = MAKELONG(newFixedWidth, HIWORD(AccessedFixedWidth)); break;
      case 4: DFSTypeFixedWidth = MAKELONG(newFixedWidth, HIWORD(DFSTypeFixedWidth)); break;
    }
  }
  else
  {
    switch (column->CustomData)
    {
      case 1: CreatedFixedWidth = MAKELONG(LOWORD(CreatedFixedWidth), newFixedWidth); break;
      case 2: ModifiedFixedWidth = MAKELONG(LOWORD(ModifiedFixedWidth), newFixedWidth); break;
      case 3: AccessedFixedWidth = MAKELONG(LOWORD(AccessedFixedWidth), newFixedWidth); break;
      case 4: DFSTypeFixedWidth = MAKELONG(LOWORD(DFSTypeFixedWidth), newFixedWidth); break;
    }
  }
...
CPluginFSDataInterface::ColumnWidthWasChanged(BOOL leftPanel, const CColumn *column, int newWidth)
{
  if (leftPanel)
  {
    switch (column->CustomData)
    {
      case 1: CreatedWidth = MAKELONG(newWidth, HIWORD(CreatedWidth)); break;
      case 2: ModifiedWidth = MAKELONG(newWidth, HIWORD(ModifiedWidth)); break;
      case 3: AccessedWidth = MAKELONG(newWidth, HIWORD(AccessedWidth)); break;
      case 4: DFSTypeWidth = MAKELONG(newWidth, HIWORD(DFSTypeWidth)); break;
    }
  }
  else
  {
    switch (column->CustomData)
    {
      case 1: CreatedWidth = MAKELONG(LOWORD(CreatedWidth), newWidth); break;
      case 2: ModifiedWidth = MAKELONG(LOWORD(ModifiedWidth), newWidth); break;
      case 3: AccessedWidth = MAKELONG(LOWORD(AccessedWidth), newWidth); break;
      case 4: DFSTypeWidth = MAKELONG(LOWORD(DFSTypeWidth), newWidth); break;
    }
  }
}
...
*/
    ...
    else sprintf(buffer, "Selected: <%d> files and <%d> directories", selectedFiles, selectedDirs);
